<template>
  <div class="bg-gray-800 rounded-lg overflow-hidden border border-gray-700 hover:border-gray-600 transition-colors">
    <!-- Collapsible Header -->
    <button 
      class="w-full p-4 text-left flex items-center justify-between hover:bg-gray-750 transition-colors" 
      @click="$emit('toggle')"
    >
      <div class="flex items-center flex-1 min-w-0">
        <!-- Status Indicator -->
        <div :class="[
          'w-3 h-3 rounded-full mr-3 flex-shrink-0',
          getStatusColor(),
          hasAlerts && alerts.some(a => a.severity === 'critical') ? 'animate-pulse' : ''
        ]"></div>
        
        <!-- Apiary Info -->
        <div class="flex-1 min-w-0">
          <h3 class="font-semibold text-lg truncate">{{ apiary.name }}</h3>
          <div class="text-sm text-gray-400 truncate">
            {{ apiary.hive_count || 0 }} hives • 
            {{ apiary.sensor_count || 0 }} sensors
            <span v-if="hasAlerts" :class="getAlertTextColor()">
              • {{ alerts.length }} {{ alerts.length === 1 ? 'alert' : 'alerts' }}
            </span>
          </div>
          <div class="text-xs text-gray-500 mt-1 flex items-center">
            <span v-if="apiary.address" class="truncate mr-2">
              📍 {{ apiary.address }}
            </span>
            <span v-if="apiary.latitude && apiary.longitude" class="text-gray-400 flex-shrink-0">
              {{ parseFloat(apiary.latitude).toFixed(4) }}, {{ parseFloat(apiary.longitude).toFixed(4) }}
            </span>
          </div>
        </div>
      </div>

      <!-- Right Side Info -->
      <div class="flex items-center space-x-3 flex-shrink-0">
        <!-- Alert Badges (when closed) -->
        <div v-if="!isExpanded && hasAlerts" class="flex space-x-2">
          <span 
            v-if="criticalAlerts.length > 0"
            class="px-2 py-1 bg-red-600 text-white text-xs rounded-full font-medium"
          >
            {{ criticalAlerts.length }} CRITICAL
          </span>
          <span 
            v-if="warningAlerts.length > 0"
            class="px-2 py-1 bg-yellow-600 text-white text-xs rounded-full font-medium"
          >
            {{ warningAlerts.length }} WARNING
          </span>
        </div>

        <!-- Status Info -->
        <div class="text-right text-sm">
          <div class="flex items-center space-x-1">
            <div :class="apiary.is_active ? 'bg-green-400' : 'bg-gray-400'" class="w-2 h-2 rounded-full"></div>
            <span :class="apiary.is_active ? 'text-green-400' : 'text-gray-400'" class="text-xs">
              {{ apiary.is_active ? 'Active' : 'Inactive' }}
            </span>
          </div>
          <div class="text-xs text-gray-500">
            {{ formatDate(apiary.installation_date) }}
          </div>
        </div>

        <!-- Expand/Collapse Arrow -->
        <svg :class="[
          'w-4 h-4 text-gray-400 transition-transform',
          isExpanded ? 'rotate-180' : ''
        ]" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
        </svg>
      </div>
    </button>
    
    <!-- Expandable Content -->
    <div :class="['accordion-content', isExpanded ? 'open' : '']">
      <div class="px-4 pb-4">
        
        <!-- Alerts Section (when expanded) -->
        <div v-if="hasAlerts" class="mb-4 space-y-3">
          <h5 class="text-sm font-medium text-gray-300 mb-3 mt-3">Active Alerts</h5>
          
          <!-- Critical Alerts -->
          <div
            v-for="alert in criticalAlerts"
            :key="alert.id"
            class="bg-gray-900 rounded-lg p-4 border-l-4 border-red-500"
          >
            <div class="flex justify-between items-start">
              <div class="flex-1">
                <div class="flex items-center mb-2">
                  <div class="w-3 h-3 bg-red-500 rounded-full mr-3"></div>
                  <h6 class="font-semibold">{{ alert.title }}</h6>
                  <span class="px-2 py-1 bg-red-900/30 text-red-400 rounded-full text-xs font-medium ml-3">
                    CRITICAL
                  </span>
                </div>
                <p class="text-gray-300 text-sm mb-2">{{ alert.message || 'Critical condition detected' }}</p>
                <div class="text-xs text-gray-500">
                  {{ alert.hiveName }} • {{ formatTime(alert.created_at) }}
                </div>
              </div>
              <button class="text-green-400 hover:text-green-300 text-sm font-medium ml-4">
                Resolve
              </button>
            </div>
          </div>

          <!-- Warning Alerts -->
          <div
            v-for="alert in warningAlerts"
            :key="alert.id"
            class="bg-gray-900 rounded-lg p-4 border-l-4 border-yellow-500"
          >
            <div class="flex justify-between items-start">
              <div class="flex-1">
                <div class="flex items-center mb-2">
                  <div class="w-3 h-3 bg-yellow-500 rounded-full mr-3"></div>
                  <h6 class="font-semibold">{{ alert.title }}</h6>
                  <span class="px-2 py-1 bg-yellow-900/30 text-yellow-400 rounded-full text-xs font-medium ml-3">
                    WARNING
                  </span>
                </div>
                <p class="text-gray-300 text-sm mb-2">{{ alert.message || 'Warning condition detected' }}</p>
                <div class="text-xs text-gray-500">
                  {{ alert.hiveName }} • {{ formatTime(alert.created_at) }}
                </div>
              </div>
              <button class="text-green-400 hover:text-green-300 text-sm font-medium ml-4">
                Resolve
              </button>
            </div>
          </div>
        </div>

        <!-- Hives Section -->
        <div>
          <div class="flex items-center justify-between mb-3">
            <h4 class="text-sm font-medium text-gray-300">Hives</h4>
            <span class="text-xs text-gray-400">{{ apiary.hive_count || 0 }} total</span>
          </div>
          
          <!-- No Hives State -->
          <div v-if="!apiary.hives || apiary.hives.length === 0" class="text-center py-6 bg-gray-900 rounded-lg border-2 border-dashed border-gray-700">
            <svg class="w-8 h-8 mx-auto mb-2 text-gray-500" viewBox="0 0 55 56" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M51 31C53.2091 31 55 32.7909 55 35V52C55 54.2091 53.2091 56 51 56H4C1.79086 56 0 54.2091 0 52V35C3.54346e-07 32.7909 1.79086 31 4 31H51ZM19.5 36C18.6716 36 18 36.6716 18 37.5C18 38.3284 18.6716 39 19.5 39H37.5C38.3284 39 39 38.3284 39 37.5C39 36.6716 38.3284 36 37.5 36H19.5Z" fill="currentColor"/>
              <path d="M51 13C53.2091 13 55 14.7909 55 17V24C55 26.2091 53.2091 28 51 28H4C1.79086 28 1.12747e-07 26.2091 0 24V17C0 14.7909 1.79086 13 4 13H51ZM18.5 17C17.6716 17 17 17.6716 17 18.5C17 19.3284 17.6716 20 18.5 20H36.5C37.3284 20 38 19.3284 38 18.5C38 17.6716 37.3284 17 36.5 17H18.5Z" fill="currentColor"/>
              <path d="M51 0C53.2091 0 55 1.79086 55 4V6C55 8.20914 53.2091 10 51 10H4C1.79086 10 3.22133e-08 8.20914 0 6V4C0 1.79086 1.79086 0 4 0H51ZM18.5 3C17.6716 3 17 3.67157 17 4.5C17 5.32843 17.6716 6 18.5 6H36.5C37.3284 6 38 5.32843 38 4.5C38 3.67157 37.3284 3 36.5 3H18.5Z" fill="currentColor"/>
            </svg>
            <p class="text-xs text-gray-500">No hives assigned</p>
            <p class="text-xs text-gray-600 mt-1">Click to add hives</p>
          </div>
          
          <!-- Hives Grid -->
          <div v-else class="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-3">
            <div
              v-for="hive in apiary.hives"
              :key="hive.id"
              class="min-w-0"
            >
              <HiveCard
                :hive="hive"
                @click="$emit('hive-click', hive, apiary)"
              />
            </div>
          </div>
        </div>

        <!-- Apiary Footer Actions -->
        <div class="flex justify-between items-center pt-3 mt-3 border-t border-gray-700">
          <div class="flex items-center space-x-2">
            <span class="text-xs text-gray-400">
              {{ apiary.installation_date ? `Created ${formatDate(apiary.installation_date)}` : 'Recently created' }}
            </span>
          </div>
          <button 
            @click.stop="$emit('apiary-click', apiary)"
            class="text-xs text-blue-400 hover:text-blue-300 transition-colors"
          >
            View Apiary Details →
          </button>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
import { computed } from 'vue'
import HiveCard from '../hive/HiveCard.vue'

// Props
const props = defineProps({
  apiary: {
    type: Object,
    required: true
  },
  alerts: {
    type: Array,
    default: () => []
  },
  isExpanded: {
    type: Boolean,
    default: false
  }
})

// Events
const emit = defineEmits(['toggle', 'apiary-click', 'hive-click'])

// Computed properties
const hasAlerts = computed(() => props.alerts.length > 0)

const criticalAlerts = computed(() => 
  props.alerts.filter(alert => alert.severity === 'critical')
)

const warningAlerts = computed(() => 
  props.alerts.filter(alert => alert.severity === 'warning')
)

// Helper functions
const getStatusColor = () => {
  if (criticalAlerts.value.length > 0) return 'bg-red-500'
  if (warningAlerts.value.length > 0) return 'bg-yellow-500'
  if (!props.apiary.is_active) return 'bg-gray-500'
  return 'bg-green-400'
}

const getAlertTextColor = () => {
  if (criticalAlerts.value.length > 0) return 'text-red-400'
  if (warningAlerts.value.length > 0) return 'text-yellow-400'
  return 'text-gray-400'
}

const formatDate = (dateString) => {
  if (!dateString) return 'Recently created'
  const date = new Date(dateString)
  return date.toLocaleDateString('en-US', { 
    month: 'short', 
    day: 'numeric',
    year: 'numeric'
  })
}

const formatTime = (date) => {
  if (!date) return 'Unknown'
  const now = new Date()
  const diff = now - new Date(date)
  const minutes = Math.floor(diff / (1000 * 60))
  const hours = Math.floor(diff / (1000 * 60 * 60))
  const days = Math.floor(diff / (1000 * 60 * 60 * 24))
  
  if (minutes < 1) return 'Just now'
  if (minutes < 60) return `${minutes}m ago`
  if (hours < 24) return `${hours}h ago`
  return `${days}d ago`
}
</script>

<style scoped>
.accordion-content {
  max-height: 0;
  overflow: hidden;
  transition: max-height 0.3s ease-out;
}

.accordion-content.open {
  max-height: 2000px;
  transition: max-height 0.3s ease-in;
}

.rotate-180 {
  transform: rotate(180deg);
}

.transition-transform {
  transition: transform 0.3s ease;
}

/* Hover effects */
.hover\:bg-gray-750:hover {
  background-color: rgb(55, 65, 81, 0.8);
}

/* Pulse animation */
.animate-pulse {
  animation: pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite;
}

@keyframes pulse {
  0%, 100% {
    opacity: 1;
  }
  50% {
    opacity: 0.5;
  }
}

/* Mobile responsiveness */
@media (max-width: 640px) {
  .grid-cols-\[repeat\(auto-fit\,minmax\(250px\,1fr\)\)\] {
    grid-template-columns: 1fr;
  }
}
</style>