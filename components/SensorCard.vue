      ...
      <div class="flex items-center space-x-4 flex-1">
        ...
        <div class="relative">
          <div class="w-12 h-12 rounded-lg flex items-center justify-center bg-gray-700">
            <!-- Icons chosen by canonicalType -->
            <svg v-if="canonicalType === 'temperature'" class="w-6 h-6 text-red-400" fill="currentColor" viewBox="0 0 20 20">
            ...
            <svg v-else-if="canonicalType === 'humidity'" class="w-6 h-6 text-blue-400" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M10 3s-6 5.419-6 8a6 6 0 1012 0c0-2.581-6-8-6-8z"/>
            </svg>
            <svg v-else-if="canonicalType === 'weight'" class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 48 50">
            ...
            <!-- Default icon for any other/unknown types -->
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-1">
            <h3 class="font-medium text-white truncate">
              {{ sensor.name || `${formatSensorType(canonicalType) } Sensor` }}
            </h3>
            <!-- Sensor Type Badge with matching colors -->
            <span :class="[
              'px-2 py-0.5 text-xs font-medium rounded-full',
              getSensorTypeBadgeColor(canonicalType)
            ]">
              {{ formatSensorType(canonicalType) }}
            </span>
          </div>
...
/**
 * 1) Normalize raw type -> lowercase
 * 2) Map common aliases / hardware models -> canonical types
 *    Add to this map as you introduce new sensor models.
 */
...
const aliasMap = {
  ...
  dht11: 'temperature',
  dht22: 'temperature',
  th: 'temperature',        // temp/humidity combo when recorded as temp
  // humidity
  humidity: 'humidity',
  hum: 'humidity',
  rh: 'humidity',
...
// Enhanced styling function for badges
...
// Labels and value formatting now use canonicalType
const formatSensorType = (type) => {
  ...
  return labels[type] || (type ? type.charAt(0).toUpperCase() + type.slice(1) : 'Sensor')
}
...
const formatSensorValue = (value, unit) => {
  ...
  switch (canonicalType.value) {
    ...
    case 'weight':
      // assuming raw value is grams; adjust if your DB already stores kg
